...
- [x] `go.mod`: `bubbletea v1.3.10`, `bubbles v1.0.0`, `huh v1.0.0`,
      `lipgloss v1.1.0`; `go 1.24.2` as the graph requires. No `toolchain`
      line.
- [x] `CONTRIBUTING.md`: note the exception to "standard library first" and
...
- [x] `progress.go`: `Phase` (Update, Download, Extract, InstallEssential,
...
- [x] `parseProgress(reader io.Reader, progress Progress)`: line scanner with
...
- [x] `testdata/mmdebstrap-verbose.log`: an excerpt of the recorded stream
...
- [x] `Mmdebstrap.Run`: add `--verbose`; tee output to the parser, to
      ...
      complete.
- [x] `Builder.Build` reports WriteLock and PlaceTarball; `export.Place` gets
      a `func(copiedBytes, totalBytes int64)` callback used on the copy path.
- [x] `Version = "0.2.0"`.

## Task 3: `internal/form`

- [x] `field.go`: `Kind` (Input, Select, MultiSelect, Confirm), `Option{Value,
...
- [x] `locales.go`: curated list with labels.
- [x] Validators reuse `recipe`'s rules: export `recipe.CheckImageName`,
...
- [x] `cli`: `Interface` selection (`--plain`, terminals, `TERM=dumb`), a
      `terminalChecker` field for tests.
- [x] `cli/plainform.go`: asks `form.Fields()` through `Prompt`: numbered
      options for selects, `y/n` for confirms, numbers-or-names for
      multi-selects. `init` and `edit` share `runForm`.
- [x] `cli/edit.go`: load, validate, `FromRecipe`, form, `ToRecipe`,
      `writeRecipe`. Missing file → exit 1 with the `init` hint.
- [x] `cli/plainprogress.go`: `Progress` that prints phase lines and every
      10% of a measured phase to stderr; replaces the raw stream.
- [x] Rewrite `init_test.go` for the new questions; add `edit_test.go`;
      `build_test.go` checks the phase lines.
- [x] README: commands table gains `edit` and `--plain`; the quick start shows
      the form (as a description, not a transcript).

## Task 5: `internal/tui` form

- [x] `form.go`: `RunForm(ctx, fields, values, in io.Reader, out io.Writer)`:
...
- [x] `theme.go`: one lipgloss theme, colors that read on dark and light
      terminals.
- [x] Tests: the built form has one field per table row with the right kind
      and default; a scripted key sequence through a fake terminal
      (`tea.WithInput`/`WithOutput`) completes `init` with the defaults.
- [x] `cli` wires the full-screen path for `init` and `edit`.

## Task 6: `internal/tui` build screen

- [x] `build.go`: `buildModel` with the phase list, spinner, per-phase
      ...
      in, out)`.
- [x] `cli.runBuild`: full-screen path runs `Builder.Build` in a goroutine
      with a channel-backed `Progress`, the screen in the foreground; after it
      returns, prints the v0.1 summary in plain text.
- [x] Tests: model transitions from events, percentage arithmetic, view
      contains titles, percentage and last log line; narrow width does not
      panic.

## Task 7: verification

- [x] Integration test additions: phases arrive in order with a positive
      download total; every catalog package exists in the `main` and
      `universe` indexes of all three releases (checked against the archive,
      which is cheaper than installing them and proves the same thing for
      `init`'s purposes).
- [x] Under a pseudo-terminal on the build host: `init` driven by scripted
      keys writes the recipe; `build` renders the screen and prints the
      summary; `frostroot build > log` uses plain automatically.
- [ ] Manual, in Windows Terminal: `init`, `edit`, `build` at 80×24 and
      narrow; resize mid-build; Ctrl-C mid-download; `--plain` for each.
      Record results in the spec's revision history.
- [x] `docs/superpowers/specs/2026-09-14-frostroot-design.md`: mark the TUI
